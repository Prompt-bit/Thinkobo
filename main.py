...
def not_found(_e):
    ...
    return htmlmin.minify(html, remove_empty_space=True), 404
...
def dashboard():
    ...
    try:
        ...
        html = render_template("dashboard.html", user=user, courses=courses)
        return htmlmin.minify(html, remove_empty_space=True)
    finally:
        conn.close()
...
def report_account_page():
    html = render_template("report_account.html")
    return htmlmin.minify(html, remove_empty_space=True)
...
def twofa_page():
    ...
    html = render_template("2fa.html", user=target, pending=bool(pending), mode=mode, next_path=next_path)
    return htmlmin.minify(html, remove_empty_space=True)
...
def admin_login_submit():
    ...
    if not password or not secrets.compare_digest(password, expected):
        ...
        html = render_template("admin_login.html", error="Invalid password.")
        return htmlmin.minify(html, remove_empty_space=True), 401
...
def admin_panel():
    ...
    try:
        ...
        html = render_template("admin.html", csrf=csrf, reports=reports, reviews=reviews)
        return htmlmin.minify(html, remove_empty_space=True)
    finally:
        conn.close()
...
def reviews_page():
    ...
    html = render_template("reviews.html", site=site, reviews=reviews)
    return htmlmin.minify(html, remove_empty_space=True)
